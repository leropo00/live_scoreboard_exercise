...
-  Update score takes as parameter current score for both teams and follows the rules:
    ...
    - The difference between current and new score must be one of the following scenarios:
       -  Same score -> Both scores are the same, the scoreboard permits updating the same score 
                and doesn't throw an exception for such operation.
       -  Goal scored -> Score is larger by exactly 1 for one of the teams and same for the the other team.
       -  Goal rejected -> Team that scored the last goal, has score smaller by exactly 1, 
